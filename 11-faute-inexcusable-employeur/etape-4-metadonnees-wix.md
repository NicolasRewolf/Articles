# Métadonnées Wix — Faute inexcusable de l'employeur (#11)
...
- **Alt** : « Sécurité sur un chantier — faute inexcusable de l'employeur et indemnisation de la victime ».
...
- `seoSlug` : `faute-inexcusable-employeur-indemnisation`
- `richContent` : `ricos.min.json` (≈ 36 Ko ; garde-fou : 51 nœuds, 14 titres, 8 FAQ, 9 liens internes SELF, 6 externes BLANK+rel).

## ⚠️ Action SEO complémentaire (côté Wix) — redirection 301
L'ancienne URL `/actualites-indemnisations-accidents-travail/faute-inexcusable-accident-travail` renvoie un **404** (dépubliée). Si elle captait encore des positions en GSC, poser une **redirection 301** vers la nouvelle notion `/post/faute-inexcusable-employeur-indemnisation` pour récupérer l'historique et le jus de liens. À vérifier dans la Search Console avant.
